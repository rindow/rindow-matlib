...
public:
    static void execute(int32_t trans,int32_t m,int32_t n,T *x, int32_t incX, T *a, int32_t ldA)
    {
        if(m <= 0 || n <= 0) {
            return;
        }
    ...
    int32_t trans,int32_t m,int32_t n,float *x, int32_t incX,float *a, int32_t ldA)
{
    ...
    Multiply<float>::execute(trans, m, n, x, incX, a, ldA);
...
{
    RINDOW_BEGIN_CLEAR_EXCEPTION;
    Multiply<double>::execute(trans, m, n, x, incX, a, ldA);
    RINDOW_END_CLEAR_EXCEPTION;
}
}
