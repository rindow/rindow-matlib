...
#define RINDOW_MATLIB_NOTEQUAL_TEMPLATE(i,n,x,incX,y,incY) \
    for(i=0; i<n; i++) { \
        if(x[i*incX] != y[i*incY]) \
            y[i*incY] = 1; \
        else \
    ...
    int32_t dtype, int32_t n, void *x, int32_t incX, void *y, int32_t incY)
{
    ...
    switch (dtype)
    {
        case rindow_matlib_dtype_bool: {
            ...
            #pragma omp parallel for
            for(i=0;i<n;i++) {
                if((typed_x[i*incX] && !typed_y[i*incY]) ||  // 1 == 0
                    (!typed_x[i*incX] && typed_y[i*incY])    // 0 == 1
                ) {
                    typed_y[i*incY] = 1;
                } else {
                    typed_y[i*incY] = 0;
        ...
        case rindow_matlib_dtype_int8: {
            ...
            int32_t i;
            #pragma omp parallel for
            RINDOW_MATLIB_NOTEQUAL_TEMPLATE(i,n,typed_x,incX,typed_y,incY)
            break;
        }
